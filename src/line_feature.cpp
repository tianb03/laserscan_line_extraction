...
//设定正余弦等参数
void LineFeature::setCosSinData(const std::vector<double>& bearings, const std::vector<double>& cos_value,
                                const std::vector<double>& sin_value, const std::vector<unsigned int>& index)
{
...
bool LineFeature::detectline(const int start, const int num)
{
  ...
  {
    //到直线的垂直距离
    error1 = fabs(((m_least.a) * range_data_.xs[k] + (m_least.b) * range_data_.ys[k] + m_least.c)) /
             sqrt((1 + (m_least.a) * (m_least.a)));
...
void LineFeature::cleanline()
{
  ...
  for (p = 0; p < (m_line.size() - 1); p++)
  {
    ...
    if (m >= n)
    {
      ...
      {
        line_temp = m_iter;
        error1 = fabs(((m_line[p].a) * range_data_.xs[m_iter] + (m_line[p].b) * range_data_.ys[m_iter] + m_line[p].c)) /
                 sqrt((1 + (m_line[p].a) * (m_line[p].a)));
        error2 = fabs(((m_line[q].a) * range_data_.xs[m_iter] + (m_line[q].b) * range_data_.ys[m_iter] + m_line[q].c)) /
                 sqrt((1 + (m_line[q].a) * (m_line[q].a)));
        if (error1 > error2)
        {
          break;
        }
...
void LineFeature::generate(std::vector<gline>& temp_line2)
{
  ...
  for (int i = 0; i < m_line.size(); i++)
  {
    ...
    {
      endpoint1.x =
          (range_data_.xs[m] / m_line[i].a + range_data_.ys[m] - m_line[i].c) / (m_line[i].a + 1.0 / (m_line[i].a));
    ...
    {
      endpoint2.x =
          (range_data_.xs[n] / m_line[i].a + range_data_.ys[n] - m_line[i].c) / (m_line[i].a + 1.0 / (m_line[i].a));
